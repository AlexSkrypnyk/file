...
final class ReplacerTest extends UnitTestCase {
  ...
  /**
   * Assert that two directories have identical file contents.
   *
   * @param string $expected
   *   Path to directory with expected contents.
   * @param string $actual
   *   Path to directory with actual contents.
   */
  protected function assertDirectoriesIdentical(string $expected, string $actual): void {
    $expected_files = $this->getRelativeFiles($expected);
    $actual_files = $this->getRelativeFiles($actual);

    $this->assertSame(
      $expected_files,
      $actual_files,
      'Directory file lists do not match.'
    );

    foreach ($expected_files as $file) {
      $expected_content = file_get_contents($expected . DIRECTORY_SEPARATOR . $file);
      $actual_content = file_get_contents($actual . DIRECTORY_SEPARATOR . $file);

      $this->assertSame(
        $expected_content,
        $actual_content,
        sprintf('File contents do not match: %s', $file)
      );
    }
  }

  /**
   * Get list of files in a directory with relative paths.
   *
   ...
   */
  protected function getRelativeFiles(string $directory): array {
