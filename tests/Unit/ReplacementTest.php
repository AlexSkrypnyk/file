...
final class ReplacementTest extends UnitTestCase {
  ...
  public static function dataProviderApply(): \Iterator {
    // Plain string matchers (no regex).
    yield 'plain string match' => [
      'foo',
      'bar',
      'foo baz foo',
      'bar baz bar',
    ...
    yield 'plain string multiple occurrences' => [
      'old',
      'new',
      'old value old again',
      'new value new again',
  ...
  public function testPlainStringWithRegexExclusion(): void {
    ...
    // 'foo' matches regex exclusion /^fo/, so no replacement.
    $this->assertFalse($result);
    $this->assertSame('foo test foo', $content);
  }
